...
public class LegionHandler {
    ...
    public static void test(LivingAttackEvent event) {
        if (event.getSource().getEntity() instanceof Player p && p.getMainHandItem().is(ItemTags.BANNERS) && !p.level.isClientSide()) {
    ...
    public static void interact(PlayerInteractEvent.EntityInteract e) {
        if (e.getTarget() instanceof OwnableEntity ownable && e.getEntity().getMainHandItem().is(ItemTags.BANNERS)) {
            Entity entity = e.getTarget();
            if (ownable.getOwner() == e.getEntity() && entity instanceof LivingEntity ent && ownable.getOwner() instanceof LivingEntity owner) {
                ...
                if (cap.isPresent() && cap.resolve().get().getSummoner(ent.level) != null) {
                    e.getEntity().displayClientMessage(Component.translatable("pocketlegion.already"), true);
                    return;
                }
                PetInfo link = null;
                if (sum.isPresent()) {
                    link = sum.resolve().get().add(ent, owner);
                }
                if (cap.isPresent()) {
                    cap.resolve().get().setSummoner(owner);
                    cap.resolve().get().setInfo(link);
                }
                e.getEntity().displayClientMessage(Component.translatable("pocketlegion.added"), true);
            }
        }
    }
